...
## script to run coloc for Olink and COVID GWAS
## Maik Pietzner 03/08/2021
rm(list=ls())
...
if(chr.s == 23){
  res.olink        <- paste0("zcat /home/mdp50/rds/rds-rjh234-mrc-epid/Studies/People/Ellie/Olink/GWAS/output/formatted/combined_chrX/",
...
## define minimum p
p.min                     <- min(res.olink$Pvalue, na.rm=T)

#------------------------------------------------#
##-- proceed only if evidence for association --##
#------------------------------------------------#
...
if(p.min < 1e-4){
  ...
  if(p.min < 1e-5){
    ...
    require(data.table)
    tmp           <- fread(paste0("tmp_input//tmp.", olink, ".", chr.s, ".", pos.s, ".", pos.e, ".dosage"), sep=" ", header=T, data.table=F)
    ## transpose
    ...
    ## make sure that all SNPs match to the SNP data
    res.all <- subset(res.all, rsid %in% tmp.info$rsid)
    ...
    res.coloc <- lapply(c("A2", "B1", "B2", "C2"), function(x){
      ...
      ## add to the data
      foo <- subset(res.all, rsid %in% c(ii, jj))[, c("MarkerName", "rsid", "Allele1.olink", "Allele2.olink", "Effect", "StdErr", "Pvalue", 
                                                            "Allele1.covid", "Allele2.covid", paste(c("Effect", "StdErr", "Pvalue"), x, sep=".")), drop=F]
      ## create output: first signal is top olinkLogic, second signal is top Covid
      # tmp        <- rbind(cbind(res, foo[1,]), cbind(res, foo[nrow(foo),])) 
      tmp        <- cbind(res, foo[1,])
      ...
      ## check whether protein singal is preserved in the overlap
      tmp$ld.sens  <- cor(pheno[,tmp.info$id[which(tmp.info$rsid == ii)]], pheno[,tmp.info$id[which(tmp.info$rsid == res.olink$rsid[which.max(abs(res.olink$Effect/res.olink$StdErr))])]])^2
    ...
    ## write to file
    write.table(res.coloc, paste("output_olink/results.coloc", olink, chr.s, pos.s, pos.e, "txt", sep="."), sep="\t", row.names=F)
    ...
    if(nrow(tmp)>0){
      for(j in 1:nrow(tmp)){
        ...
        png(paste0("graphics_olink/", olink, ".", tmp[j,8], ".", chr.s, ".", pos.s, ".", pos.e, ".png"), width=16, height=8, units="cm", res=200)
        ...
        plot.regional.coloc(res.all, tmp[j,], pheno, tmp[j,8], olink, tmp.info)
        dev.off()
      }
    }
  }else{
    ...
    ## write to file
    write.table(data.frame(pheno=olink, outcome=NA), paste("output_olink//results.coloc", olink, chr.s, pos.s, pos.e, "txt", sep="."), sep="\t", row.names=F)
  }
